...
const MediaLocationCard = ({
  ...
  return (
    <section className="rounded-xl border border-white/10 bg-slate-950/55 p-3" aria-label="Media location">
      ...
      {hasCoordinates ? (
        <dl className={`mt-3 grid gap-2 text-[11px] ${compact ? "grid-cols-1" : "sm:grid-cols-2"}`}>
          ...
          <div>
            ...
            <dd className="mt-0.5 font-mono text-slate-200">{Number(location.latitude).toFixed(6)}, {Number(location.longitude).toFixed(6)}</dd>
          </div>
          <div>
            <dt className="text-slate-500">Accuracy</dt>
            <dd className="mt-0.5 text-slate-200">{location.accuracyMeters ? `±${Math.round(Number(location.accuracyMeters))} metres` : "Not supplied"}</dd>
          </div>
          <div className="sm:col-span-2">
      ...
      {(onRetryGps || (hasCoordinates && onRefreshAddress) || mapUrl || (canRemove && onRemove)) ? (
        <div className="mt-3 flex flex-wrap gap-2">
          {onRetryGps ? <button type="button" onClick={onRetryGps} className="inline-flex items-center gap-1 rounded-lg border border-white/15 px-2 py-1 text-[11px] text-slate-200"><RefreshCw size={12} /> Retry GPS</button> : null}
          {hasCoordinates && onRefreshAddress ? <button type="button" onClick={onRefreshAddress} className="inline-flex items-center gap-1 rounded-lg border border-cyan-300/20 px-2 py-1 text-[11px] text-cyan-100"><RefreshCw size={12} /> Refresh Address</button> : null}
          {mapUrl ? <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded-lg border border-emerald-300/20 px-2 py-1 text-[11px] text-emerald-100"><ExternalLink size={12} /> Open Location</a> : null}
